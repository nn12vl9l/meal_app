    ...
    <div class="container lg:w-1/2 md:w-4/5 w-11/12 mx-auto mt-8 px-8 bg-white shadow-md">
        ...
        <form action="{{ route('posts.store') }}" method="POST" enctype="multipart/form-data"
            ...
            <div class="mb-4">
                ...
                <input type="text" name="title" class="rounded-md shadow-sm border-gray-300 focus:border-indigo-300 focus:ring focus:ring-indigo-200 focus:ring-opacity-50 w-full py-2 px-3"
            ...
            <div class="mb-4">
                ...
                @foreach ($categories as $category)
                    <label class="block"><input class="mr-2" type="radio" name="category_id"
                            value="{{ $category->id }}" @if (old('category_id') == $category->id) checked @endif>{{ old('category_id', $category->name) }}</label>
                @endforeach
            </div>
            <div class="mb-4">
            ...
            <div class="mb-4">
                <label class="block text-gray-700 text-sm mb-2" for="image">食事の画像</label>
